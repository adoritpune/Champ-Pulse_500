...
//			}
	...
	{	
			...
			{	
					...
					lcdput(2,0,ar35);//ar35);  //ar35[]="PREFLOWXXX:XXXXXXSEC";
					if(weldmode=='G')
					...
					lcdput(3,0,ar36);//ar36);  //ar36[]="BURNBACKXX:XXXXXXSEC";
					if(weldmode=='G')
		...
		else
		{
			...
			if(lcdpage==2)
				{	
					...
					lcdput(2,0,ar35);//ar35);   //change 09052023
					if(weldmode=='G')
					lcd_disp_hex_dec(para.migmode.preflow*10);
					if(weldmode=='P')
					lcd_disp_hex_dec(para.pmigmode.preflow*10);
...
void MainScreenShow(void) 
{
	...
	if((weldmode=='G') ||(weldmode=='P'))
	{
		if(iProgramLoadLoc==211 && weldmode=='G')
		{
			lcdput(6,0,cMatMigTxt[5]);  
			lcdput(6,88,cGasTxt[2]);	
